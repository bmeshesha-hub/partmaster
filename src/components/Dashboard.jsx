...
export default function Dashboard({ data, onNavigate, onOpenSettings }) {
  ...
  function progressAction() {
   ...
   return <div className="space-y-6">
    ...
    <section className="rounded-3xl border border-emerald-200 bg-white p-5 shadow-panel sm:p-6" aria-label="Master parts enrichment progress"><div className="grid gap-6 lg:grid-cols-[auto_1fr_auto] lg:items-center"><ProgressDonut done={chartDone} remaining={chartRemaining} label={chartLabel} /><div><p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">Visual completion</p><h3 className="mt-1 text-2xl font-black text-slate-950">Master data enrichment</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{number(chartDone)} unique parts have product facts and {number(chartRemaining)} still need enrichment.</p><div className="mt-4 flex flex-wrap gap-3 text-xs font-black"><span className="rounded-full bg-emerald-100 px-3 py-1.5 text-emerald-800">Enriched: {number(chartDone)}</span><span className="rounded-full bg-amber-100 px-3 py-1.5 text-amber-800">Remaining: {number(chartRemaining)}</span>{catalogProgress?.live && <span className="rounded-full bg-cyan-100 px-3 py-1.5 text-cyan-800">Verified in master: {number(verifiedEnrichmentRows)}</span>}{catalogProgress?.live && <span className="rounded-full bg-amber-100 px-3 py-1.5 text-amber-800">Needs review: {number(reviewEnrichmentRows)}</span>}{trackedSchedule && <span className="rounded-full bg-blue-100 px-3 py-1.5 text-blue-800">Row batch: {number(trackedSchedule.processed_rows)} done</span>}</div></div><div className="flex min-w-52 flex-col gap-2">{scheduleNeedsResume ? <button type="button" disabled={Boolean(actionBusy)} onClick={progressAction} className="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-3 text-sm font-black text-white disabled:opacity-50"><Play size={17} />Resume batch</button> : <button type="button" disabled={Boolean(actionBusy) || activeRowBatch} onClick={progressAction} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white disabled:opacity-50"><Timer size={17} />{trackedSchedule ? trackedSchedule.remaining_rows ? activeRowBatch ? "Open active batch" : "Run next batch" : "Open enrichment" : "Set up row schedule"}</button>}<button type="button" onClick={() => onNavigate("enrichment")} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50"><Gauge size={17} />Open enrichment</button></div></div></section>
