...
function ProgressDonut({ done, remaining, label }) {
  const total = Math.max(0, Number(done || 0) + Number(remaining || 0));
  const percent = total ? Math.min(100, (Number(done || 0) / total) * 100) : 0;
  const percentLabel = percent > 0 && percent < 1 ? percent.toFixed(1) : Math.round(percent);
  const radius = 43;
  const circumference = 2 * Math.PI * radius;
  return <div className="relative h-44 w-44 shrink-0" aria-label={`${number(done)} parts enriched, ${percentLabel}% of the Master, with ${number(remaining)} remaining`} role="img"><svg viewBox="0 0 110 110" className="h-full w-full -rotate-90"><circle cx="55" cy="55" r={radius} fill="none" stroke="#e2e8f0" strokeWidth="11" /><circle cx="55" cy="55" r={radius} fill="none" stroke="#10b981" strokeWidth="11" strokeLinecap="round" strokeDasharray={`${(percent / 100) * circumference} ${circumference}`} className="transition-all duration-700" /></svg><div className="absolute inset-0 grid place-items-center text-center"><div><p className="text-2xl font-black text-slate-950">{number(done)}</p><p className="text-[10px] font-black uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 text-[10px] font-bold text-emerald-700">{percentLabel}% covered</p></div></div></div>;
}
...
export default function Dashboard({ data, onNavigate, onOpenSettings }) {
  ...
  const loadLiveProgress = useCallback(async () => {
    const [sourceAudit, pipelineJobs, rowScheduleResult, masterStats] = await Promise.all([localDataApi.pipelineSources(), localDataApi.pipelineJobs(), localDataApi.enrichmentSchedules(), localDataApi.masterStats()]);
    setCatalogProgress({ ...sourceAudit, generated_at: new Date().toISOString(), live: true, job: pipelineJobs.jobs?.[0] || null, batchSchedules: rowScheduleResult.schedules || [], verifiedEnrichmentRows: Number(masterStats.stats?.enriched_candidates || 0), reviewEnrichmentRows: Number(masterStats.stats?.awaiting_review || 0) });
  }, []);
  ...
  // Scheduled enrichment remains live while waiting for its next batch.
  const activeRowBatch = rowSchedules.some((schedule) => schedule.enabled && (Number(schedule.remaining_rows || 0) > 0 || ["queued", "running", "waiting"].includes(schedule.job_status)));
  ...
  const verifiedEnrichmentRows = Number(catalogProgress?.verifiedEnrichmentRows || 0);
  const reviewEnrichmentRows = Number(catalogProgress?.reviewEnrichmentRows || 0);
  const totalPages = Number(progress.source_pages || 0);
  const trackedSchedule = rowSchedules[0] || null;
  // Master completion is measured against deduplicated parts, matching the
  // master-data table and the headline above. Row schedules are operational
  // progress and are shown separately below.
  ...
  function progressAction() {
    if (!trackedSchedule) { onOpenSettings?.(); return; }
    if (scheduleNeedsResume) { controlRowSchedule(trackedSchedule, "resume-job"); return; }
    if (activeRowBatch) { onNavigate("enrichment"); return; }
    if (Number(trackedSchedule.remaining_rows || 0)) { controlRowSchedule(trackedSchedule, "run"); return; }
    onNavigate("enrichment");
  }

   return <div className="space-y-6">
    <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-blue-950 to-emerald-950 text-white shadow-panel"><div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1.35fr_0.65fr] lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-300">Live pipeline progress</p><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${catalogProgress?.live ? "bg-emerald-400 text-emerald-950" : "bg-white/10 text-slate-300"}`}>{catalogProgress?.live ? "Live from this Mac" : "Published snapshot"}</span></div><h3 className="mt-3 text-2xl font-black tracking-tight sm:text-3xl">{catalogProgress ? `${number(enrichedRows)} of ${number(masterParts)} unique parts enriched` : "Loading catalog progress…"}</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Raw-row scanning and product enrichment are separate stages. This dashboard reports both so a slow enrichment lookup cannot masquerade as scan progress.</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><div><div className="flex justify-between text-xs font-bold text-slate-300"><span>Raw rows scanned</span><span>{number(scannedRows)} / {number(rawRows)}</span></div><div className="mt-2 h-2.5 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-cyan-400 transition-all" style={{ width: `${rawRows ? Math.min(100, scannedRows / rawRows * 100) : 0}%` }} /></div></div><div><div className="flex justify-between text-xs font-bold text-slate-300"><span>Unique parts enriched</span><span>{number(enrichedRows)} / {number(masterParts)}</span></div><div className="mt-2 h-2.5 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-emerald-400 transition-all" style={{ width: `${masterParts ? Math.min(100, enrichedRows / masterParts * 100) : 0}%` }} /></div></div></div><div className="mt-3 flex flex-wrap justify-between gap-2 text-xs font-bold text-slate-300"><span>{rawRows ? (Math.min(100, scannedRows / rawRows * 100)).toFixed(1) : "0.0"}% raw scan complete</span><span>{number(enrichmentRemaining)} unique parts still need facts</span></div></div><div className="grid grid-cols-2 gap-3"><button type="button" onClick={() => onNavigate("master")} className="rounded-xl bg-white px-4 py-4 text-left text-sm font-semibold text-slate-900 hover:bg-cyan-50"><Database className="mb-3 text-emerald-600" size={21} />Open Master Data<ArrowRight className="mt-3" size={17} /></button><button type="button" onClick={() => onNavigate("enrichment")} className="rounded-xl border border-white/20 bg-white/10 px-4 py-4 text-left text-sm font-semibold text-white hover:bg-white/15"><Gauge className="mb-3 text-cyan-300" size={21} />Open Enrichment<ArrowRight className="mt-3" size={17} /></button></div></div></section>
    <section className="rounded-3xl border border-emerald-200 bg-white p-5 shadow-panel sm:p-6" aria-label="Master parts enrichment progress"><div className="grid gap-6 lg:grid-cols-[auto_1fr_auto] lg:items-center"><ProgressDonut done={chartDone} remaining={chartRemaining} label={chartLabel} /><div><p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">Visual completion · product facts</p><h3 className="mt-1 text-2xl font-black text-slate-950">How much of the Master is enriched?</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{number(chartDone)} of {number(chartDone + chartRemaining)} unique Master parts have product facts. This is enrichment progress—not final approval.</p><div className="mt-4 flex flex-wrap gap-3 text-xs font-black"><span className="rounded-full bg-emerald-100 px-3 py-1.5 text-emerald-800">Facts found: {number(chartDone)}</span><span className="rounded-full bg-amber-100 px-3 py-1.5 text-amber-800">Facts still needed: {number(chartRemaining)}</span>{catalogProgress?.live && <span className="rounded-full bg-cyan-100 px-3 py-1.5 text-cyan-800">Verified & saved: {number(verifiedEnrichmentRows)}</span>}{catalogProgress?.live && <span className="rounded-full bg-amber-100 px-3 py-1.5 text-amber-800">Needs review: {number(reviewEnrichmentRows)}</span>}{trackedSchedule && <span className="rounded-full bg-blue-100 px-3 py-1.5 text-blue-800">Rows processed: {number(trackedSchedule.processed_rows)}</span>}</div></div><div className="flex min-w-52 flex-col gap-2">{scheduleNeedsResume ? <><button type="button" disabled={Boolean(actionBusy)} onClick={progressAction} className="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-3 text-sm font-black text-white disabled:opacity-50"><Play size={17} />Resume batch</button><p className="text-center text-xs font-semibold text-amber-700">This batch needs attention before it can continue.</p></> : <><button type="button" disabled={Boolean(actionBusy)} onClick={progressAction} title={activeRowBatch ? "An enrichment batch is already running." : activePipeline ? "CSV processing is still in progress." : !trackedSchedule ? "Create a schedule to start enrichment." : !trackedSchedule.remaining_rows ? "All scheduled rows are complete." : "Run the next enrichment batch."} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"><Timer size={17} />{trackedSchedule ? trackedSchedule.remaining_rows ? activeRowBatch ? "Open active batch" : "Run next batch" : "Open enrichment" : "Set up row schedule"}</button>{activeRowBatch ? <p className="text-center text-xs font-semibold text-cyan-700">Enrichment is already running. Open the active batch to monitor it.</p> : activePipeline ? <p className="text-center text-xs font-semibold text-amber-700">CSV processing is ongoing. Enrichment will be available when it finishes.</p> : !trackedSchedule ? <p className="text-center text-xs font-semibold text-slate-500">No enrichment schedule is set up yet.</p> : !trackedSchedule.remaining_rows ? <p className="text-center text-xs font-semibold text-emerald-700">All scheduled rows are complete.</p> : null}</>}</div></div></section>
