...
export default function Dashboard({ data, onNavigate }) {
  ...
  useEffect(() => {
    ...
    async function loadProgress() {
      ...
      try {
        ...
        if (!cancelled) setCatalogProgress({ ...sourceAudit, generated_at: new Date().toISOString(), live: true, job: pipelineJobs.jobs?.[0] || null });
  ...
  return <div className="space-y-6">
    ...
    {!catalogProgress ? <div className="grid min-h-40 place-items-center rounded-3xl border border-slate-200 bg-white"><LoaderCircle className="animate-spin text-brand-600" size={28} /></div> : <>
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-panel"><header className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 bg-slate-50 px-5 py-4 sm:px-6"><div><p className="flex items-center gap-2 text-xs font-black uppercase tracking-wide text-brand-700"><Files size={16} />Progress by raw CSV</p><h3 className="mt-1 text-lg font-black">Size, rows, completion and remaining work</h3><p className="mt-1 text-xs leading-5 text-slate-500">“In Master” means its known OEM identities are consolidated. “Needs facts” and “pages left” are separate enrichment work.</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-black ${pipelineStatus === "completed" ? "bg-emerald-100 text-emerald-800" : pipelineStatus === "running" ? "bg-blue-100 text-blue-800" : "bg-slate-200 text-slate-700"}`}>Pipeline {pipelineStatus.replaceAll("_", " ")}</span></header><div className="overflow-x-auto"><table className="min-w-full text-xs"><thead className="border-b border-slate-200 bg-white text-left uppercase tracking-wide text-slate-500"><tr>{["CSV source", "Size", "Raw rows", "Rows remaining", "Unique raw parts", "In Master", "Missing Master", "Needs facts", "Pages left", "Status"].map((heading) => <th key={heading} className="whitespace-nowrap px-3 py-3">{heading}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{sources.map((source) => { const indexed = source.import_status === "indexed" || source.is_indexed === true; const rowsLeft = indexed ? 0 : Number(source.raw_rows || 0); return <tr key={source.source_file} className="hover:bg-slate-50"><td className="max-w-64 px-3 py-3 font-bold text-slate-800"><span className="break-all">{source.source_file}</span></td><td className="whitespace-nowrap px-3 py-3">{formatBytes(source.source_bytes)}</td><td className="px-3 py-3 font-semibold">{number(source.raw_rows)}</td><td className={`px-3 py-3 font-black ${rowsLeft ? "text-amber-700" : "text-emerald-700"}`}>{number(rowsLeft)}</td><td className="px-3 py-3">{number(source.unique_parts)}</td><td className="px-3 py-3 font-black text-blue-700">{number(source.master_parts)}</td><td className="px-3 py-3 font-black text-red-700">{number(source.remaining_master_parts)}</td><td className="px-3 py-3 font-semibold text-violet-700">{number(source.remaining_fact_parts)}</td><td className="px-3 py-3 font-semibold text-cyan-700">{number(source.pending_source_pages)}</td><td className="px-3 py-3"><span className={`whitespace-nowrap rounded-full px-2.5 py-1 font-black ${indexed ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{indexed ? "In Master" : "Needs processing"}</span></td></tr>; })}</tbody></table>{!sources.length && <div className="px-6 py-10 text-center text-sm text-slate-500">Per-CSV metrics will appear after the source inventory is published or the local service is connected.</div>}</div></section>
    </>}
