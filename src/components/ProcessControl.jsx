...
const ACTIVE = ["queued", "running", "processing", "waiting"];
const typeConfig = {
  ...
  fitment: { label: "Fitment mapping", color: "emerald", pause: localDataApi.pauseFitmentEnrichment, resume: localDataApi.resumeFitmentEnrichment },
  autopilot: { label: "Intelligence Autopilot", color: "cyan", pause: localDataApi.pauseAutopilot, resume: localDataApi.resumeAutopilot },
};
const toneClasses = { indigo: ["bg-indigo-50", "text-indigo-700", "bg-indigo-500"], violet: ["bg-violet-50", "text-violet-700", "bg-violet-500"], emerald: ["bg-emerald-50", "text-emerald-700", "bg-emerald-500"], cyan: ["bg-cyan-50", "text-cyan-700", "bg-cyan-500"] };
...
function detailStats(type, job) {
  ...
  if (type === "fitment") return [["Mapped", job.mapped_count], ["Unresolved", job.unresolved_count], ["Failed", job.failed_count]];
...
function ProcessTable({ items, onControl, busy }) {
  return <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-panel"><table className="min-w-[900px] w-full text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Process</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Progress</th><th className="px-4 py-3">Results</th><th className="px-4 py-3">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{items.map(({ type, job }) => { const active = ACTIVE.includes(job.status); const config = typeConfig[type]; const pct = Math.min(100, Math.max(0, Number(progress(type, job)) || 0)); const work = currentWork(type, job, Number(job.processed_count || job.completed_count || job.scanned_rows || 0), Number(job.queued_count || job.total_rows || job.requested_parts || 0)); const stats = detailStats(type, job).filter(([, value]) => value != null); return <tr key={`${type}-${job.id}`} className="align-middle hover:bg-slate-50"><td className="max-w-xs px-4 py-3"><p className="font-black text-slate-900">{job.name || "Unnamed process"}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-slate-500">{config.label}</p><p className="mt-1 text-slate-500">{job.started_at ? new Date(job.started_at).toLocaleString() : "Not started"}</p></td><td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 font-bold ${active ? "bg-blue-50 text-blue-700" : job.status === "completed" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{job.status?.replaceAll("_", " ")}</span><p className="mt-2 max-w-48 text-slate-500">{actionLabel(type, job, active)}</p></td><td className="w-64 px-4 py-3"><div className="flex justify-between font-bold text-slate-700"><span>{work.processed.toLocaleString()} / {work.total.toLocaleString()}</span><span>{Math.round(pct)}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${toneClasses[config.color][2]}`} style={{ width: `${pct}%` }} /></div><p className="mt-1 text-slate-500">{work.label}</p></td><td className="px-4 py-3"><div className="flex gap-3">{stats.map(([label, value]) => <span key={label} className="whitespace-nowrap"><b className="block text-sm text-slate-900">{Number(value || 0).toLocaleString()}</b><span className="text-slate-500">{label}</span></span>)}</div></td><td className="px-4 py-3">{active ? <button type="button" disabled={busy} onClick={() => onControl(type, "pause", job.id)} className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 font-bold text-amber-800">Pause</button> : ["paused", "failed"].includes(job.status) ? <button type="button" disabled={busy} onClick={() => onControl(type, "resume", job.id)} className="rounded-lg bg-brand-600 px-3 py-2 font-bold text-white">Resume</button> : "—"}</td></tr>; })}</tbody></table></div>;
}

export default function ProcessControl({ mode = "monitor", onModeChange: _onModeChange }) {
  const [items, setItems] = useState([]); const [schedules, setSchedules] = useState([]); const [liveSummary, setLiveSummary] = useState({}); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [available, setAvailable] = useState(true); const [logs, setLogs] = useState([]);
  const load = useCallback(async () => {
    try { const [pipeline, enrichment, fitment, autopilot, rowSchedules, sources] = await Promise.all([localDataApi.pipelineJobs(), localDataApi.enrichmentJobs(), localDataApi.fitmentEnrichmentJobs(), localDataApi.autopilotJobs(), localDataApi.enrichmentSchedules(), localDataApi.pipelineSources()]); const fitmentItems = (fitment.jobs || []).map((job) => ({ type: "fitment", job: { ...job, queued_count: job.total_count, processed_count: job.processed_count, completed_count: job.processed_count } })); const nextItems = [...(pipeline.jobs || []).map((job) => ({ type: "pipeline", job })), ...(enrichment.jobs || []).map((job) => ({ type: "enrichment", job })), ...fitmentItems, ...(autopilot.jobs || []).map((job) => ({ type: "autopilot", job }))].sort((a, b) => String(b.job.created_at).localeCompare(String(a.job.created_at))); setItems(nextItems); setSchedules(rowSchedules.schedules || []); setLiveSummary(sources.summary || {}); setAvailable(true); setError(""); }
  ...
  const activeSchedules = useMemo(() => schedules.filter((schedule) => schedule.enabled), [schedules]);
  useEffect(() => { const timer = window.setInterval(load, 4000); return () => window.clearInterval(timer); }, [load]);
  ...
  async function controlSchedule(schedule, enabled) { setBusy(true); try { await localDataApi.updateEnrichmentSchedule(schedule.id, { enabled }); await load(); } catch (requestError) { setError(requestError.message); } finally { setBusy(false); } }
  if (!available && !loading) return <LocalWorkspaceUnavailable />;
  if (!loading && items.length && mode !== "logs") return <div className="space-y-4">
    <section className="sticky top-0 z-20 -mx-2 rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-sm backdrop-blur sm:p-4"><div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">{[["Active", active.length], ["Queued", items.filter(({ job }) => job.status === "queued").length], ["Needs review", liveSummary.review_enrichment_rows ?? items.reduce((sum, { job }) => sum + Number(job.review_count || 0) + Number(job.conflict_count || 0), 0)], ["Failed", items.filter(({ job }) => job.status === "failed").length], ["Verified & saved", liveSummary.verified_enrichment_rows ?? 0], ["Success rate", `${items.length ? Math.round(items.filter(({ job }) => job.status === "completed").length / items.length * 100) : 0}%`], ["Rows processed", liveSummary.enriched_rows ?? items.reduce((sum, { job }) => sum + Number(job.processed_count || job.completed_count || 0), 0)], ["Remaining", liveSummary.remaining_enrichment_rows ?? items.reduce((sum, { job }) => sum + Math.max(0, Number(job.queued_count || 0) - Number(job.processed_count || job.completed_count || 0)), 0)]].map(([label, value]) => <div key={label} className="rounded-xl bg-slate-50 px-3 py-2"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 text-lg font-black tabular-nums text-slate-900">{displayMetricValue(value)}</p>{label === "Success rate" && <p className="mt-0.5 text-[9px] text-slate-400">completed jobs ÷ total jobs</p>}</div>)}</div></section>
    <div className="flex items-center justify-between gap-3"><div><h3 className="text-lg font-bold text-slate-900">Process history</h3><p className="text-sm text-slate-500">Dense monitoring view · live data refreshes automatically.</p></div><button type="button" onClick={load} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700"><RefreshCw size={16} />Refresh</button></div>
    <ProcessTable items={items} onControl={control} busy={busy} />
  </div>;
  return <div className="space-y-6">
    ...
    {activeSchedules.length > 0 && <section className="rounded-3xl border border-cyan-200 bg-cyan-50/50 p-5 shadow-panel sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-700">Queued work</p><h3 className="mt-1 text-xl font-black text-slate-950">Scheduled enrichment queue</h3><p className="mt-1 text-sm text-slate-600">These schedules continue automatically in 1,000-row batches with a 5-minute pause.</p></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-black text-cyan-800 ring-1 ring-cyan-200">{activeSchedules.length} active queue(s)</span></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{activeSchedules.map((schedule) => <article key={schedule.id} className="rounded-2xl border border-cyan-100 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="font-black text-slate-950">{schedule.name}</h4><p className="mt-1 text-xs text-slate-500">{schedule.dataset_name} · {Number(schedule.batch_size).toLocaleString()} rows every {Number(schedule.interval_minutes).toLocaleString()} minutes</p></div><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-black uppercase text-emerald-800">Queued / active</span></div><div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-xl bg-emerald-50 p-3"><p className="text-[10px] font-bold uppercase text-emerald-700">Done</p><p className="mt-1 font-black text-emerald-950">{Number(schedule.processed_rows || 0).toLocaleString()}</p></div><div className="rounded-xl bg-amber-50 p-3"><p className="text-[10px] font-bold uppercase text-amber-700">Queued left</p><p className="mt-1 font-black text-amber-950">{Number(schedule.remaining_rows || 0).toLocaleString()}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-bold uppercase text-slate-600">Next batch</p><p className="mt-1 truncate text-xs font-black text-slate-900">{schedule.next_run_at ? new Date(String(schedule.next_run_at).replace(" ", "T")).toLocaleString() : "Starting soon"}</p></div></div><button type="button" disabled={busy} onClick={() => controlSchedule(schedule, false)} className="mt-4 rounded-xl border border-slate-300 px-3 py-2 text-xs font-black text-slate-700 disabled:opacity-50">Pause queue</button></article>)}</div></section>}
