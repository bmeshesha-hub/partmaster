...
export default function ProcessControl({ mode = "monitor", onModeChange }) {
  const [items, setItems] = useState([]); const [schedules, setSchedules] = useState([]); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [available, setAvailable] = useState(true); const [logs, setLogs] = useState([]);
  const load = useCallback(async () => {
    try { const [pipeline, enrichment, fitment, autopilot, rowSchedules] = await Promise.all([localDataApi.pipelineJobs(), localDataApi.enrichmentJobs(), localDataApi.fitmentEnrichmentJobs(), localDataApi.autopilotJobs(), localDataApi.enrichmentSchedules()]); const fitmentItems = (fitment.jobs || []).map((job) => ({ type: "fitment", job: { ...job, queued_count: job.total_count, processed_count: job.processed_count, completed_count: job.processed_count } })); const nextItems = [...(pipeline.jobs || []).map((job) => ({ type: "pipeline", job })), ...(enrichment.jobs || []).map((job) => ({ type: "enrichment", job })), ...fitmentItems, ...(autopilot.jobs || []).map((job) => ({ type: "autopilot", job }))].sort((a, b) => String(b.job.created_at).localeCompare(String(a.job.created_at))); setItems(nextItems); setSchedules(rowSchedules.schedules || []); setAvailable(true); setError(""); }
  ...
  useEffect(() => { if (!active.length && !activeSchedules.length) return undefined; const timer = window.setInterval(load, 4000); return () => window.clearInterval(timer); }, [active.length, activeSchedules.length, load]);
