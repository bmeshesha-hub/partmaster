...
export default function ProcessControl({ mode = "monitor", onModeChange }) {
  ...
  useEffect(() => { if (!active.length && !schedules.some((schedule) => schedule.enabled)) return undefined; const timer = window.setInterval(load, 4000); return () => window.clearInterval(timer); }, [active.length, schedules, load]);
  ...
  return <div className="space-y-6">
    ...
    {schedules.length > 0 && <section className="rounded-3xl border border-cyan-200 bg-cyan-50/50 p-5 shadow-panel sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-700">Queued work</p><h3 className="mt-1 text-xl font-black text-slate-950">Scheduled enrichment queue</h3><p className="mt-1 text-sm text-slate-600">These schedules continue automatically in 1,000-row batches with a 5-minute pause.</p></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-black text-cyan-800 ring-1 ring-cyan-200">{schedules.filter((schedule) => schedule.enabled).length} active queue(s)</span></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{schedules.map((schedule) => <article key={schedule.id} className="rounded-2xl border border-cyan-100 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="font-black text-slate-950">{schedule.name}</h4><p className="mt-1 text-xs text-slate-500">{schedule.dataset_name} · {Number(schedule.batch_size).toLocaleString()} rows every {Number(schedule.interval_minutes).toLocaleString()} minutes</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${schedule.enabled ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>{schedule.enabled ? "Queued / active" : "Paused"}</span></div><div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-xl bg-emerald-50 p-3"><p className="text-[10px] font-bold uppercase text-emerald-700">Done</p><p className="mt-1 font-black text-emerald-950">{Number(schedule.processed_rows || 0).toLocaleString()}</p></div><div className="rounded-xl bg-amber-50 p-3"><p className="text-[10px] font-bold uppercase text-amber-700">Queued left</p><p className="mt-1 font-black text-amber-950">{Number(schedule.remaining_rows || 0).toLocaleString()}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-bold uppercase text-slate-600">Next batch</p><p className="mt-1 truncate text-xs font-black text-slate-900">{schedule.enabled ? (schedule.next_run_at ? new Date(String(schedule.next_run_at).replace(" ", "T")).toLocaleString() : "Starting soon") : "Paused"}</p></div></div><button type="button" disabled={busy} onClick={() => controlSchedule(schedule, !schedule.enabled)} className="mt-4 rounded-xl border border-slate-300 px-3 py-2 text-xs font-black text-slate-700 disabled:opacity-50">{schedule.enabled ? "Pause queue" : "Resume queue"}</button></article>)}</div></section>}
