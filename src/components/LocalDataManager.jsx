...
export default function LocalDataManager() {
  ...
  const [mappingValidationRunning, setMappingValidationRunning] = useState(false);
  ...
  async function validateVehicleMappings() {
    setMappingValidationRunning(true);
    setError("");
    setMessage("");
    try {
      const result = await localDataApi.validateVehicleMappings();
      setVehicleMappings(result);
      setMessage(`Vehicle mapping validated: ${Number(result.vehicles || 0).toLocaleString()} ePID vehicles and ${Number(result.aliases || 0).toLocaleString()} source aliases reloaded. Both workbook tabs are active in lookup and validation.`);
    } catch (requestError) {
      setError(requestError.message);
      await refreshLocal();
    } finally {
      setMappingValidationRunning(false);
    }
  }
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-panel sm:p-6">
      ...
      <div className={`mt-4 rounded-xl border px-4 py-3 ${vehicleMappings?.available ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className={`flex items-center gap-2 text-sm font-semibold ${vehicleMappings?.available ? "text-emerald-900" : "text-amber-900"}`}>{vehicleMappings?.available && <ShieldCheck size={16} />}{vehicleMappings?.available ? "Vehicle mapping validated and active" : "Vehicle mapping validation required"}</p><p className="mt-1 text-xs text-slate-600">{vehicleMappings?.available ? `${Number(vehicleMappings.vehicles).toLocaleString()} ePID vehicles · ${Number(vehicleMappings.aliases).toLocaleString()} source aliases${Number(vehicleMappings.mpsov_aliases) ? ` · ${Number(vehicleMappings.mpsov_aliases).toLocaleString()} MPSOV lookup names` : ""}` : vehicleMappings?.validation?.errors?.[0] || "Validate Vehicle Mapping ePID.xlsx before using fitment mapping."}</p>{vehicleMappings?.validation?.status === "passed" && <p className="mt-1 text-xs font-semibold text-emerald-700">Both tabs validated: the wide VCDB/ARI/NADA crosswalk supplies canonical vehicles, and the MPSOV Source tab supplies lookup aliases.</p>}{vehicleMappings?.available && Number(vehicleMappings.mpsov_enriched_applications) > 0 && <p className="mt-1 text-xs font-semibold text-emerald-700">MPSOV details applied to {Number(vehicleMappings.mpsov_enriched_applications).toLocaleString()} master applications and {Number(vehicleMappings.mpsov_enriched_candidates || 0).toLocaleString()} review candidates.</p>}</div><button type="button" onClick={validateVehicleMappings} disabled={mappingValidationRunning} className="inline-flex shrink-0 items-center gap-2 rounded-lg border border-emerald-300 bg-white px-3 py-2 text-xs font-bold text-emerald-800 disabled:opacity-50">{mappingValidationRunning ? <LoaderCircle className="animate-spin" size={14} /> : <ShieldCheck size={14} />}{mappingValidationRunning ? "Validating…" : "Validate / reload mapping"}</button></div></div>
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_1fr_auto]"><label className="text-sm font-medium text-slate-700">Inbox parts CSV/TSV<select value={selectedFile} onChange={(event) => setSelectedFile(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5"><option value="">Select a parts file…</option>{files.filter((file) => file.kind !== "vehicle_reference").map((file) => <option key={file.name} value={file.name}>{file.name} · {formatBytes(file.bytes)}</option>)}</select></label><label className="text-sm font-medium text-slate-700">Dataset name<input value={datasetName} onChange={(event) => setDatasetName(event.target.value)} placeholder="Optional friendly name" className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5" /></label><button type="button" onClick={startImport} disabled={!selectedFile || ["queued", "importing"].includes(job?.status)} className="mt-auto inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{["queued", "importing"].includes(job?.status) ? <LoaderCircle className="animate-spin" size={17} /> : <FileInput size={17} />}{job?.status === "importing" ? "Importing…" : "Import file"}</button></div>
