...
export default function LocalDataManager() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-panel sm:p-6">
      ...
      <div className={`mt-4 rounded-xl border px-4 py-3 ${vehicleMappings?.available ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}><div className="flex flex-wrap items-center justify-between gap-2"><div><p className={`text-sm font-semibold ${vehicleMappings?.available ? "text-emerald-900" : "text-amber-900"}`}>{vehicleMappings?.available ? "Vehicle mapping reference active" : "Vehicle mapping reference not loaded"}</p><p className="mt-1 text-xs text-slate-600">{vehicleMappings?.available ? `${Number(vehicleMappings.vehicles).toLocaleString()} ePID vehicles · ${Number(vehicleMappings.aliases).toLocaleString()} source aliases${Number(vehicleMappings.mpsov_aliases) ? ` · ${Number(vehicleMappings.mpsov_aliases).toLocaleString()} MPSOV lookup names` : ""}` : "Run npm run vehicle:mapping:extract, then restart the local service."}</p>{vehicleMappings?.available && Number(vehicleMappings.mpsov_enriched_applications) > 0 && <p className="mt-1 text-xs font-semibold text-emerald-700">MPSOV details applied to {Number(vehicleMappings.mpsov_enriched_applications).toLocaleString()} master applications and {Number(vehicleMappings.mpsov_enriched_candidates || 0).toLocaleString()} review candidates.</p>}</div>{vehicleMappings?.available && <span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-emerald-700">Optional enrichment source</span>}</div></div>
