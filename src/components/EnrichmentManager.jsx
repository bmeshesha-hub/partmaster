...
function FullDatasetPipeline() {
  ...
  return <section className="overflow-hidden rounded-3xl border border-indigo-200 bg-white shadow-panel">
    ...
    <div className="p-5 sm:p-7">
      ...
      <section className="mt-6 rounded-2xl border-2 border-blue-300 bg-gradient-to-br from-blue-50 via-white to-emerald-50 p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">The answer</p><h4 className="mt-1 text-lg font-black text-slate-950">Raw CSV inventory and enrichment</h4><p className="mt-1 max-w-3xl text-xs leading-5 text-slate-600">Every discovered raw file is included. “Done” means a raw row has finished enrichment; “left” means it has not.</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-black ${Number(sourceCoverage.unindexed_files) ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{Number(sourceCoverage.unindexed_files || 0) ? `${Number(sourceCoverage.unindexed_files).toLocaleString()} file(s) not imported yet` : "All discovered files imported"}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><CoverageMetric label="Raw rows" value={sourceCoverage.known_raw_rows} detail={`${Number(sourceCoverage.discovered_files || 0).toLocaleString()} raw CSV files found`} /><CoverageMetric label="Enriched / done" value={sourceCoverage.enriched_rows} detail="Rows with a finished enrichment result" tone="text-emerald-700" /><CoverageMetric label="Left to enrich" value={sourceCoverage.remaining_enrichment_rows} detail="Rows still waiting for enrichment" tone="text-amber-700" /></div>
