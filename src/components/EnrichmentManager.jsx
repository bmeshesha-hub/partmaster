...
function SourceCoverageTable({ sources, busy, active, onContinue }) {
  ...
  return <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200"><header className="bg-slate-50 px-4 py-3"><h4 className="text-sm font-bold">Completion by raw source CSV</h4><p className="mt-0.5 text-xs text-slate-500">A file can be fully consolidated into master while still needing product facts or online evidence. Invalid rows are held out when no usable manufacturer + OEM identity can be formed; they are not silently deleted.</p></header><div className="overflow-x-auto"><table className="min-w-full text-xs"><thead className="border-y border-slate-200 bg-white text-left uppercase tracking-wide text-slate-500"><tr>{["Source", "Status", "Raw rows", "Invalid held out", "Usable", "Unique raw", "In master", "Missing", "With facts", "Pages left", ""].map((heading) => <th key={heading} className="whitespace-nowrap px-3 py-2">{heading}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{sources.map((source) => {
...
function candidateActionPlan(candidate) {
  const missing = candidateMissingFields(candidate);
  if (["not_found", "failed"].includes(candidate.status) && candidate.source_url) return "Fetch source page";
  if (candidate.status === "conflict") return "Resolve conflict";
  if (missing.includes("OEM number") && candidate.source_url) return "Fetch OEM number";
  if (missing.length) return "Complete missing fields";
  if (isBulkApprovable(candidate)) return "Approve or review";
...
function FullDatasetPipeline() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setJobs(nextJobs);
      const catalogResult = await localDataApi.pipelineCatalog(catalogQuery);
  ...
  async function scheduleAllRemainingRows() {
    setBusy(true); setPipelineError("");
    try {
      const result = await localDataApi.createEnrichmentSchedule({
        datasetId: "all",
        name: "All raw CSV rows — 1,000 every 5 minutes",
        batchSize: 1000,
        intervalMinutes: 5,
      });
      await load();
      setPipelineError("");
      window.alert(`Scheduled ${Number(result.remainingRows || 0).toLocaleString()} remaining rows across all imported raw CSV files. Each batch is 1,000 rows with a 5-minute pause.`);
    } catch (error) { setPipelineError(error.message); }
    finally { setBusy(false); }
  }
  ...
  return <section className="overflow-hidden rounded-3xl border border-indigo-200 bg-white shadow-panel">
    <header className="flex flex-wrap items-start justify-between gap-4 bg-gradient-to-r from-indigo-950 via-slate-950 to-cyan-950 px-5 py-6 text-white sm:px-7"><div className="max-w-3xl"><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-cyan-300"><Database size={15} />All raw CSV coverage</p><h3 className="mt-2 text-2xl font-bold">How much raw data is done?</h3><p className="mt-2 text-sm leading-6 text-slate-300">This count covers every CSV, TSV, and text source found in the local inbox, including newly added Honda ATV data.</p></div><div className="grid min-w-56 gap-2"><button type="button" disabled={busy || !Number(sourceCoverage.remaining_enrichment_rows)} onClick={scheduleAllRemainingRows} className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50"><Play size={16} />Run all remaining rows</button><p className="text-center text-[11px] text-slate-300">1,000 rows, then 5-minute pause</p>{active ? <button type="button" disabled={busy} onClick={() => control("pause")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50"><Pause size={16} />Pause scan</button> : <button type="button" disabled={busy} onClick={() => start(false)} className="text-xs font-bold text-slate-300 underline underline-offset-4 hover:text-white">Refresh all raw CSVs</button>}</div></header>
    ...
    <div className="p-5 sm:p-7">
      ...
      <section className="mt-6 rounded-2xl border-2 border-blue-300 bg-gradient-to-br from-blue-50 via-white to-emerald-50 p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">The answer</p><h4 className="mt-1 text-lg font-black text-slate-950">Raw CSV inventory and enrichment</h4><p className="mt-1 max-w-3xl text-xs leading-5 text-slate-600">Every discovered raw file is included. “Done” means a raw row has finished enrichment; “verified” means it was accepted into master data.</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-black ${Number(sourceCoverage.unindexed_files) ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{Number(sourceCoverage.unindexed_files || 0) ? `${Number(sourceCoverage.unindexed_files).toLocaleString()} file(s) not imported yet` : "All discovered files imported"}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><CoverageMetric label="Raw rows" value={sourceCoverage.known_raw_rows} detail={`${Number(sourceCoverage.discovered_files || 0).toLocaleString()} raw CSV files found`} /><CoverageMetric label="Enriched / done" value={sourceCoverage.enriched_rows} detail="Rows with a finished enrichment result" tone="text-emerald-700" /><CoverageMetric label="Verified & saved" value={sourceCoverage.verified_enrichment_rows} detail="Accepted into master data" tone="text-emerald-700" /><CoverageMetric label="Needs review" value={sourceCoverage.review_enrichment_rows} detail="Not accepted yet" tone="text-amber-700" /><CoverageMetric label="Left to enrich" value={sourceCoverage.remaining_enrichment_rows} detail="Rows still waiting for enrichment" tone="text-amber-700" /></div>
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white"><header className="flex flex-wrap items-center justify-between gap-3 bg-slate-50 px-4 py-4"><div><h4 className="text-sm font-bold">Inspect the master catalog</h4><p className="mt-0.5 text-xs text-slate-500">Search the actual deduplicated records behind the totals. Held-out rows are explained in the source audit above.</p></div><label className="flex items-center gap-2 text-xs font-bold text-slate-500"><span className="sr-only">Search catalog</span><input value={catalogQuery} onChange={(event) => setCatalogQuery(event.target.value)} placeholder="Make, OEM, family…" className="w-56 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-800" />{catalogQuery && <button type="button" onClick={() => setCatalogQuery("")} className="text-brand-700 hover:underline">Clear</button>}</label></header>{catalog.rows?.length > 0 ? <div className="overflow-x-auto"><table className="min-w-full text-xs"><thead className="border-y border-slate-200 bg-white text-left uppercase tracking-wide text-slate-500"><tr>{["Make", "OEM number", "Family", "Description", "Occurrences", "Datasets", "Facts", "Online"].map((heading) => <th key={heading} className="whitespace-nowrap px-3 py-2">{heading}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{catalog.rows.map((part) => <tr key={part.part_key}><td className="px-3 py-2 font-bold">{part.manufacturer}</td><td className="whitespace-nowrap px-3 py-2 font-mono font-bold text-brand-700">{part.part_number}</td><td className="whitespace-nowrap px-3 py-2">{part.family_name || "Pending"}</td><td className="max-w-72 truncate px-3 py-2" title={part.description || ""}>{part.description || "—"}</td><td className="px-3 py-2 font-bold">{Number(part.occurrence_count).toLocaleString()}</td><td className="px-3 py-2">{Number(part.dataset_count).toLocaleString()}</td><td className="px-3 py-2 font-bold text-emerald-700">{Number(part.extracted_attribute_count).toLocaleString()}</td><td className="px-3 py-2 capitalize">{part.online_status.replaceAll("_", " ")}</td></tr>)}</tbody></table></div> : <p className="px-4 py-8 text-center text-sm text-slate-500">No master records match this filter.</p>}</section>
    </div>
  </section>;
}
...
export function ReviewModal({ candidate, onClose, onDecision, onFetchSource }) {
  ...
  async function applySearchResult(result) {
    ...
    setFieldSuggestions((current) => Object.entries(resolved.fields || {}).reduce((fields, [field, value]) => ({ ...fields, [field]: [...new Set([...(fields[field] || []), value])] }), current));
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4">
    <div className="max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
      ...
      {Object.keys(fieldSuggestions).some((field) => fieldSuggestions[field]?.length) && <div className="mx-5 mt-3 rounded-xl border border-violet-200 bg-violet-50 p-3"><p className="text-xs font-black uppercase tracking-wide text-violet-900">Suggested missing-field values — select to autofill</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{Object.entries(fieldSuggestions).map(([field, options]) => <div key={field}><p className="text-[11px] font-bold uppercase text-violet-700">{field === "familyName" ? "Part family" : field}</p><div className="mt-1 flex flex-wrap gap-1">{options.map((value) => <button key={`${field}-${value}`} type="button" onClick={() => { setValues((current) => ({ ...current, [field]: value })); setReviewError(""); }} className="rounded-md border border-violet-200 bg-white px-2 py-1 text-xs font-semibold text-violet-900 hover:bg-violet-100">{value}</button>)}</div></div>)}</div></div>}
...
export default function EnrichmentManager() {
  ...
  async function rejectSelected() {
    ...
    try {
      ...
      setSelectedCandidateIds([]);
      setMessage(`${rejected.toLocaleString()} selected records were rejected and removed from the active review queue. Raw source data was preserved.`);
      await refresh(); await loadCandidates();
    } catch (requestError) {
      setError(`${rejected.toLocaleString()} records were rejected before the process stopped. ${requestError.message}`);
      await refresh(); await loadCandidates();
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white shadow-panel">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-slate-200 px-5 py-4"><div><h3 className="font-semibold">Evidence review</h3><p className="mt-1 text-sm text-slate-500">{Number(candidateTotal).toLocaleString()} matching candidates; showing up to 200.{Object.values(appliedReviewFilters).some(Boolean) ? " Searching across all background jobs." : ""}</p></div><div className="flex flex-wrap items-center gap-2">{selectedCandidateIds.length > 0 && <><button type="button" disabled={bulkApproving} onClick={fetchSelectedEvidence} className="inline-flex items-center gap-2 rounded-xl border border-cyan-300 bg-cyan-50 px-3 py-2 text-sm font-bold text-cyan-800 disabled:opacity-50"><Globe2 size={16} />Mass recheck selected ({selectedCandidateIds.length})</button><button type="button" disabled={bulkApproving} onClick={() => setBulkConfirmCount(selectedCandidateIds.length)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-sm font-bold text-white disabled:opacity-50"><ListChecks size={16} />Approve selected ({selectedCandidateIds.length})</button><button type="button" disabled={bulkApproving} onClick={rejectSelected} className="inline-flex items-center gap-2 rounded-xl border border-red-300 bg-red-50 px-3 py-2 text-sm font-bold text-red-700 disabled:opacity-50">Reject selected</button></>}<select value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setSelectedCandidateIds([]); }} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">All statuses</option>{REVIEW_STATUSES.map((status) => <option key={status} value={status}>{status.replaceAll("_", " ")}</option>)}</select></div></header>
