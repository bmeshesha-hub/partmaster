...
function FullDatasetPipeline() {
  ...
  return <section className="overflow-hidden rounded-3xl border border-indigo-200 bg-white shadow-panel">
    <header className="flex flex-wrap items-start justify-between gap-4 bg-gradient-to-r from-indigo-950 via-slate-950 to-cyan-950 px-5 py-6 text-white sm:px-7"><div className="max-w-3xl"><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-cyan-300"><Database size={15} />Full-dataset intelligence pipeline</p><h3 className="mt-2 text-2xl font-bold">Process every source locally before spending bandwidth online</h3><p className="mt-2 text-sm leading-6 text-slate-300">Imports missing Honda and Harley files, ignores duplicate imports, repairs the usable rows, creates one global OEM identity, extracts attributes locally, then checks shared catalog pages in priority order.</p></div><div className="grid min-w-56 gap-2"><label className="text-xs font-bold uppercase tracking-wide text-slate-300">Online page budget<input type="number" min="0" max="5000" value={onlineBudget} onChange={(event) => setOnlineBudget(Number(event.target.value))} className="mt-1.5 w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2.5 text-sm font-bold text-white" /></label>{active ? <button type="button" disabled={busy} onClick={() => control("pause")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50"><Pause size={16} />Pause safely</button> : job?.status === "paused" || job?.status === "failed" ? <button type="button" disabled={busy} onClick={() => control("resume")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50"><Play size={16} />Resume pipeline</button> : Number(catalog.stats?.unique_parts) ? <><button type="button" disabled={busy || !Number(catalog.stats?.pending_source_pages)} onClick={() => start(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50">{busy ? <LoaderCircle className="animate-spin" size={16} /> : <Globe2 size={16} />}Continue online checks</button><button type="button" disabled={busy} onClick={() => start(false)} className="text-xs font-bold text-slate-300 underline decoration-white/30 underline-offset-4 hover:text-white">Rebuild from all source files</button></> : <button type="button" disabled={busy} onClick={() => start(false)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50">{busy ? <LoaderCircle className="animate-spin" size={16} /> : <Play size={16} />}Process all source files</button>}</div></header>
...
export default function EnrichmentManager() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white shadow-panel">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-slate-200 px-5 py-4"><div><h3 className="font-semibold">Evidence review</h3><p className="mt-1 text-sm text-slate-500">{Number(candidateTotal).toLocaleString()} matching candidates; showing up to 200.{Object.values(appliedReviewFilters).some(Boolean) ? " Searching across all background jobs." : ""}</p></div><div className="flex flex-wrap items-center gap-2">{selectedCandidateIds.length > 0 && <button type="button" onClick={() => setBulkConfirmCount(selectedCandidateIds.length)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-sm font-bold text-white"><ListChecks size={16} />Approve selected ({selectedCandidateIds.length})</button>}<select value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setSelectedCandidateIds([]); }} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">All statuses</option>{REVIEW_STATUSES.map((status) => <option key={status} value={status}>{status.replaceAll("_", " ")}</option>)}</select></div></header>
