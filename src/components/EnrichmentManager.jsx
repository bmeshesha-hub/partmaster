...
function FullDatasetPipeline() {
  ...
  async function start() {
    ...
    try { await localDataApi.startPipeline({ importMissing: true, onlineBudget }); await load(); }
    catch (error) { setPipelineError(error.message); }
  ...
  return <section className="overflow-hidden rounded-3xl border border-indigo-200 bg-white shadow-panel">
    <header className="flex flex-wrap items-start justify-between gap-4 bg-gradient-to-r from-indigo-950 via-slate-950 to-cyan-950 px-5 py-6 text-white sm:px-7"><div className="max-w-3xl"><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-cyan-300"><Database size={15} />Full-dataset intelligence pipeline</p><h3 className="mt-2 text-2xl font-bold">Process every source locally before spending bandwidth online</h3><p className="mt-2 text-sm leading-6 text-slate-300">Imports missing Honda and Harley files, ignores duplicate imports, repairs the usable rows, creates one global OEM identity, extracts attributes locally, then checks shared catalog pages in priority order.</p></div><div className="grid min-w-56 gap-2"><label className="text-xs font-bold uppercase tracking-wide text-slate-300">Online page budget<input type="number" min="0" max="5000" value={onlineBudget} onChange={(event) => setOnlineBudget(Number(event.target.value))} className="mt-1.5 w-full rounded-xl border border-white/20 bg-white/10 px-3 py-2.5 text-sm font-bold text-white" /></label>{active ? <button type="button" disabled={busy} onClick={() => control("pause")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50"><Pause size={16} />Pause safely</button> : job?.status === "paused" || job?.status === "failed" ? <button type="button" disabled={busy} onClick={() => control("resume")} className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50"><Play size={16} />Resume pipeline</button> : <button type="button" disabled={busy} onClick={start} className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950 disabled:opacity-50">{busy ? <LoaderCircle className="animate-spin" size={16} /> : <Play size={16} />}Process all source files</button>}</div></header>
    ...
    <div className="p-5 sm:p-7">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">{[
        ...
        ["Product facts", job?.attribute_facts || catalog.stats?.attribute_facts], ["Source pages", job?.source_pages], ["Online verified", job?.online_verified_parts || catalog.stats?.online_verified_parts],
