...
function changeSummary(candidate) {
  const changes = [];
  const beforeMake = String(candidate.manufacturer_raw || "").trim();
  const afterMake = proposedManufacturer(candidate);
  if (beforeMake && afterMake && beforeMake !== afterMake) changes.push(`Brand: ${beforeMake} → ${afterMake}`);
  const beforeNumber = String(candidate.part_number_raw || "").trim();
  const afterNumber = partNumber(candidate);
  if (afterNumber && beforeNumber !== afterNumber) changes.push(`OEM: ${beforeNumber || "missing"} → ${afterNumber}`);
  const beforeDescription = String(candidate.description_raw || "").trim();
  const afterDescription = String(candidate.enriched_description || "").trim();
  if (afterDescription && beforeDescription !== afterDescription) changes.push("Description enriched");
  if (candidate.side && String(candidate.side).toLowerCase() !== "unknown") changes.push(`Side: ${candidate.side}`);
  if (candidate.vehicle_mapping_method) changes.push(`Vehicle mapped: ${candidate.vehicle_mapping_method.replaceAll("_", " ")}`);
  return changes;
}
...
export default function ReviewWorkspace() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    try {
      const result = await localDataApi.enrichmentCandidates({ reviewOnly: true, status, make: brand, category, q: appliedQuery, priority, limit: 200 });
      setCandidates(result.candidates || []);
      setCandidateTotal(result.total || 0);
    } catch (error) { setNotice({ type: "error", message: error.message }); }
    finally { setLoadingQueue(false); }
  }, [appliedQuery, brand, category, connected, priority, status]);
  ...
  async function decideCandidate(decision, values) {
    if (decision === "approve") {
      const changes = changeSummary(reviewing);
      const summary = changes.length ? changes.map((change) => `• ${change}`).join("\n") : "• No visible field changes; this records the evidence decision.";
      const confirmed = window.confirm(`Approve this record into Master?\n\nBefore: ${reviewing.manufacturer_raw || "missing brand"} · ${reviewing.part_number_raw || "missing OEM"}\nAfter: ${proposedManufacturer(reviewing) || "missing brand"} · ${partNumber(reviewing) || "missing OEM"}\n\nChanges:\n${summary}\n\nOriginal source values and evidence will remain traceable.`);
      if (!confirmed) return;
    }
  ...
  function openAiValidation() {
    const selected = candidates.filter((candidate) => selectedIds.includes(candidate.id));
    if (!selected.length) {
      setNotice({ type: "error", message: "Select at least one review record before starting AI validation." });
      return;
    }
    // Open the handoff before building the prompt so a large selection never
    // makes the button look unresponsive.
    ...
    try {
      ...
      setAiPrompt("");
      setAiValidationError(`The prompt could not be generated: ${error.message || "unknown error"}`);
    }
    window.setTimeout(() => document.getElementById("ai-validation-handoff")?.scrollIntoView({ behavior: "smooth", block: "start" }), 50);
  }
  ...
  return <div className="space-y-6">
    ...
    {connected ? <section id="brand-review-queue" className="scroll-mt-24 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-panel">
      ...
      <form onSubmit={submitSearch} className="grid gap-3 border-b border-slate-200 px-5 py-4 sm:grid-cols-2 lg:grid-cols-[1.7fr_1fr_1fr_1fr_auto]">
        ...
        <label className="text-[10px] font-black uppercase tracking-wide text-slate-500">Queue order<select value={priority} onChange={(event) => { setPriority(event.target.value); setSelectedIds([]); }} className="mt-1.5 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-normal normal-case tracking-normal"><option value="impact">Highest source impact</option><option value="recent">Most recently processed</option></select></label>
